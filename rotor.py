...
def angleToSteps(angle, stepsPerRev, microsteps):
    ...
    steps = int(round(angle * stepMultiplier))
    correctedAngle =  intAngle / stepPrecision
...
def setTravelDirection(intAngle):
    if intAngle > 0:
...
def initRotor(file, rotor, acceleration, microsteps = 8, mode, subMode,
    ...
    setAccelerationRamp(file, instrument, startSpeed, maxSpeed, acceleration,
                            stepsPerRev, microsteps, degPerS)
